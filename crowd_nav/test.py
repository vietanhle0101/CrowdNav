...
def main():
    ...
    num_human_in_policy = policy.model.num_human
    print("num_humans_policy = ", policy.model.num_human)
    # multi-frame env
    ...
    if args.visualize:
        ...
        num_human_in_env = len(ob)
        print("num_human_in_env = ", num_human_in_env)
        done = False
        ...
        while not done:
            ...
            if not 'RNN' in robot.policy.name:
                ...
                ob = latest_frame(ob, num_human_in_policy)
            action = robot.act(ob)
        ...
        output_file = os.path.join(args.result_dir, 'traj_' + str(num_human_in_env))
        if args.traj:
            env.render('traj', output_file + '.png')
        else:
            env.render('video', output_file + '.gif')
